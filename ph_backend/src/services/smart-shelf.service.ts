...
export class SmartShelfService {
  ...
  async getExpiringBatches(days: number = 30, page: number = 1, limit: number = 20) {
    ...
    // Get batch IDs that already have any expiry action (DISCOUNT, RETURN_TO_VENDOR, DISPOSE)
    const processedBatches = await prisma.expiryAction_Record.findMany({
      where: {
        action: {
          in: ['DISCOUNT', 'RETURN_TO_VENDOR', 'DISPOSE'],
        },
      },
      select: {
        batchId: true,
    ...
    const processedBatchIds = [...new Set(processedBatches.map((b) => b.batchId))];
    ...
    const [batches, total] = await Promise.all([
      prisma.inventoryBatch.findMany({
        where: {
          ...
          // Exclude batches that already have any expiry action applied
          id: {
            notIn: processedBatchIds.length > 0 ? processedBatchIds : undefined,
          },
        },
        include: {
          drug: true,
