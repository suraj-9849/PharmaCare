 ...
 */
router.get('/', cacheMiddleware(CacheTTL.SHORT), async (_req: Request, res: Response) => {
  try {
...
router.get(
  '/chart',
  cacheMiddleware(CacheTTL.SHORT, (req) => cacheKeyGenerators.withQuery(req, 'dashboard-chart')),
...
router.get(
  '/top-selling',
  cacheMiddleware(CacheTTL.MEDIUM, (req) => cacheKeyGenerators.withQuery(req, 'top-selling')),
  async (req: Request, res: Response) => {
    try {
