...
import { expiryCheckService } from '../services/expiry-check.service';
...
/**
 * POST /api/alerts/check-expiry
 * Manual trigger for expiry check (for testing)
 */
router.post('/check-expiry', async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return errorResponse(res, ERROR_MESSAGES.UNAUTHORIZED, HTTP_STATUS.UNAUTHORIZED);
    }

    const expiringDrugs = await expiryCheckService.triggerManualCheck();
    ...
    if (expiringDrugs.length > 0) {
      ...
      return successResponse(
        ...
        `Found ${expiringDrugs.length} drug(s) expiring within 30 days. Email sent.`
      );
    }

    return successResponse(res, { expiringCount: 0 }, 'No drugs expiring within 30 days');
  } catch (error) {
    const message = error instanceof Error ? error.message : ERROR_MESSAGES.INTERNAL_ERROR;
    return errorResponse(res, message, HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
