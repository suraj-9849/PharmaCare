...
export function BatchSelectionCard({
  ...
  const canSelectBatch = (batchIndex: number): boolean => {
    ...
    for (let i = 0; i < batchIndex; i++) {
      ...
      const selectedFromPrev = selectedBatches.find(s => s.batchId === prevBatch.id)?.quantity || 0;
          ...
          return (
            <div
              ...
              className={`border rounded-lg p-3 ${
                isFirstBatch ? 'border-green-500 border-2 bg-green-50' :
                canSelect ? 'border-gray-300' : 'border-gray-200 bg-gray-50'
              }`}
            >
              {/* Badge Row */}
              <div className="flex items-center gap-2 mb-2">
                {isFirstBatch && (
                  <Badge className="bg-green-600">SELL FIRST</Badge>
                )}
                {!isFirstBatch && !canSelect && (
                  <Badge variant="secondary">FEFO LOCKED</Badge>
                )}
                ...
                {canSelect && (
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => updateBatchQuantity(batch.id, Math.min(batch.quantity, selectedQty + 1))}
