...
class ApiClient {
  ...
  // Invoice endpoints (Image Recognition with OpenAI GPT-4o-mini)
  invoices = {
    ...
    validate: (extractedData: unknown) =>
      this.post<unknown>('/invoices/validate', { extractedData }),
    createDrug: (drugData: unknown) => this.post<unknown>('/invoices/drugs/create', drugData),
    process: (extractedData: unknown, updatedItems?: unknown) =>
      this.post<unknown>('/invoices/process', { extractedData, updatedItems }),
  ...
  smartShelf = {
    ...
    deleteShelf: (id: string, force: boolean = false) =>
      this.delete<unknown>(`/smart-shelf/${id}${force ? '?force=true' : ''}`),
    ...
    // Return to Vendor (sends email via Resend to admin/supplier)
    returnToVendor: (data: { batchId: string; supplierId: string; collectionDays?: number }) =>
      this.post<{
        returnId: string;
        batchId: string;
        supplierId: string;
      ...
      }>('/smart-shelf/return-to-vendor', data),
  };
