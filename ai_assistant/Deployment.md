...
docker build -t DrugDesk-ai:latest .
```

## 2. Create ECR Repository (if not exists)
```sh
aws ecr create-repository --repository-name DrugDesk-ai --region ap-southeast-2
```
...
```sh
docker tag DrugDesk-ai:latest 301691475000.dkr.ecr.ap-southeast-2.amazonaws.com/DrugDesk-ai:latest
docker push 301691475000.dkr.ecr.ap-southeast-2.amazonaws.com/DrugDesk-ai:latest
```

## 5. Deploy CloudFormation Stack
```sh
aws cloudformation create-stack \
  --stack-name DrugDesk-ai-stack \
  --template-body file://cloudformation.yaml \
  --capabilities CAPABILITY_IAM \
  --region ap-southeast-2 \
  --parameters \
    ParameterKey=ImageUri,ParameterValue=301691475000.dkr.ecr.ap-southeast-2.amazonaws.com/DrugDesk-ai:latest \
...
aws ecs update-service \
  --cluster DrugDesk-ai-cluster \
  --service DrugDesk-ai-service \
...
aws elbv2 describe-load-balancers --region ap-southeast-2 --query "LoadBalancers[?contains(LoadBalancerName, 'DrugDesk-ai')].DNSName" --output text
...
aws ecs run-task \
  ...
  --task-definition DrugDesk-ai-task \
  --overrides '{
    "containerOverrides": [{
      "name": "DrugDesk-ai-container",
      "environment": [
